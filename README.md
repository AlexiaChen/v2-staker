...
### 3. 获取质押中所有用户对应的质押数额权重占比和质押时间权重占比

- `getValidStakersWithWeight()` 无输入参数，返回用户地址数组(第一个参数），返回对应用户地址的质押数额权重占比的数组(第二个参数)， 返回对应用户地址的质押时间的权重占比(第三个参数), 返回这三个数组的长度（第四个参数） ` (address [] memory, uint256 [] memory, uint256 [] memory, uint)`

> 注意，权重占比由uint256整数表示，保留18位精确的小数点。比如某个用户的某个时间占比为`0.000074324500765476`，那么返回的uint256就是 `0.000074324500765476 * 1e18 = 74324500765476`。对于抵押数额占比权重同理。 为什么是要乘以`1e18`，是因为如果不保留这么多小数，那么对比占比很低的用户就表示不出来了。比如 某个用户占比 `0.005` 乘以 `1e2`的话也就是 `0.5`，而合约的代码是不可以存储小数的。

### 4. 获取特定用户的质押数额和质押时间占比权重
...
e18，那么具体的接口参数就这样调用: `notifyRewardAmount2(stakingTokenAddress, 60*60*24*2, 1000000000000000000)` 第一个参数就是支持质押的代币的地址，对应分润模型，也是ball代币的地址，那么第二个参数就是2天的秒数，第三个参数就是这2天要瓜分的奖励代币的数额（在这里是ball代币），这样依次类推，owner想定期打钱就调用这个接口，这里打钱的奖励数额和它期望的一个时间段要符合，以上的调用例子就是2天中按一定速率瓜分1个ball代币。，如果秒数设置的是4天，那么就是4天来瓜分你要设置的奖励数额。
