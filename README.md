...
### 3. Factory的deploy接口新增了一个是否锁stake的bool参数

因为普通的LP质押流动性挖矿和单币质押分润模型，都是复用了同一个stake pool的代码，但是一些细节上又不一样，单币质押分润的池子需要锁仓，LP质押的不用锁。所以需要区分开来方便维护。

单币质押分润池子，是默认锁定48小时，48小时之后才可以提取质押。
